...
function wiziq_scheduleclass($wiziq_secretacesskey, $wiziq_access_key,
    ...
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                            $wiziq_webserviceurl.'?method=create',
                                            $requestparameters);
 ...
 */
function wiziq_get_data($courseid, $class_id, &$presenter_id, &$presenter_name, &$presenter_url,
    ...
    $wiziq_webserviceurl = $CFG->wiziq_webserviceurl;
    require_once("authbase.php");
    $wiziq_authbase = new wiziq_authbase($wiziq_secretacesskey,
                                         $wiziq_access_key);
    $method = "get_data";
    $requestparameters["signature"] = $wiziq_authbase->wiziq_generatesignature($method,
                                                                             $requestparameters);
    ...
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                                $wiziq_webserviceurl.'?method=get_data',
                                                $requestparameters);
        libxml_use_internal_errors(true);
 ...
 */
function wiziq_get_data_manage($courseid, $classids) {
    global $CFG, $DB;
    $wiziq_secretacesskey = $CFG->wiziq_secretacesskey;
    $wiziq_access_key = $CFG->wiziq_access_key;
    $wiziq_webserviceurl = $CFG->wiziq_webserviceurl;
    ...
    $requestparameters["columns"] = "class_id, status";
    $wiziq_httprequest=new wiziq_httprequest();
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                                $wiziq_webserviceurl.'?method=get_data',
 ...
 */
function wiziq_addattendee($courseid, $class_id, $attendee_id, $attendee_screen_name,
    ...
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                                $wiziq_webserviceurl.'?method=add_attendees',
                                                $requestparameters);
        libxml_use_internal_errors(true);
        $objdom = new SimpleXMLElement($xmlreturn, LIBXML_NOCDATA);
        $attribnode = (string)$objdom->attributes();
...
function wiziq_delete_class($courseid, $class_id) {
    ...
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                                $wiziq_webserviceurl.'?method=cancel',
                                                $requestparameters);
        libxml_use_internal_errors(true);
        $objdom = new SimpleXMLElement($xmlreturn, LIBXML_NOCDATA);
        $attribnode = (string)$objdom->attributes();
 ...
 * @param string $errormsg error message in case there is some error in scheduling class.
 */
function wiziq_modifyclass($courseid, $wiziq_secretacesskey, $wiziq_access_key,
      $wiziq_webserviceurl, $class_id, $title, $presenter_id, $presenter_name, $wiziq_datetime,
      $wiziqtimezone, $class_duration, $vc_language, $recording, $intro, &$attribnode, &$errormsg) {
    global $CFG, $wiziq_secretacesskey, $wiziq_access_key, $wiziq_webserviceurl;
    $wiziq_secretacesskey = $CFG->wiziq_secretacesskey;
    $wiziq_access_key = $CFG->wiziq_access_key;
    $wiziq_webserviceurl = $CFG->wiziq_webserviceurl;
    ...
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                                $wiziq_webserviceurl.'?method=modify',
                                                $requestparameters);
        libxml_use_internal_errors(true);
        $objdom = new SimpleXMLElement($xmlreturn, LIBXML_NOCDATA);
        $attribnode = (string)$objdom->attributes();
 ...
 *        
 */
function wiziq_downloadrecording($courseid, $class_id, &$download_recording_link, &$errormsg) {
    global $CFG, $wiziq_secretacesskey, $wiziq_access_key, $wiziq_webserviceurl;
    $wiziq_secretacesskey = $CFG->wiziq_secretacesskey;
    $wiziq_access_key = $CFG->wiziq_access_key;
    $wiziq_webserviceurl = $CFG->wiziq_webserviceurl;
            ...
            if ($download_status == "true") {
                ...
                if ($rec_statusnode == "true") {
                    ...
                    if (!empty($download_rec_link_path)) {
                        ...
                        add_to_log($courseid, 'wiziq', 'view download recording', '',
                                   'error : '.$errormsg);
                    }
                } else {
 ...
 *        
 */
function wiziq_download_recording($courseid, $status_xml_path,
                                  &$download_rec_link_path, &$errormsgdown) {
 ...
 */
function wiziq_getattendancereport($courseid, $class_id, $id, &$errormsg,
...
function wiziq_create_folder($courseid, $folderpath, $foldername) {
    ...
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                          $wiziq_content_webservice.'?method=create_folder',
                                          $requestparameters);
        libxml_use_internal_errors(true);
        ...
        if ($createfolderxml_status == 'ok') {
            ...
            if ($createfolderxml_status == 'true') {
                ...
                add_to_log($courseid, 'wiziq', 'add create folder', '',
...
function wiziq_content_delete($courseid, $contentid) {
    ...
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                                $wiziq_content_webservice.'?method=delete',
                                                $requestparameters);
        libxml_use_internal_errors(true);
...
function wiziq_delete_folder($courseid, $foldername, $folderpath) {
    ...
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                          $wiziq_content_webservice.'?method=delete_folder',
                                          $requestparameters);
        libxml_use_internal_errors(true);
...
function wiziq_get_contentstatus($folderpath, $foldername, $courseid) {
    ...
    try {
        $xmlreturn=$wiziq_httprequest->wiziq_do_post_request(
                                                $wiziq_content_webservice.'?method=list',
                                                $requestparameters);
        libxml_use_internal_errors(true);
...
function wiziq_attendance_time($entry_time, $wiziqclassid) {
    ...
    $timestamp = mktime($date['hour'], $date['minute'],
 ...
 * @param string $recording_url recording link for viewing the recorded class.
 */
function wiziq_get_data_by_sessioncode($courseid, $sessioncode, &$class_id, $wiziq_id,
                                       &$presenter_id, &$presenter_name, &$presenter_url,
                                       &$start_time, &$time_zone, &$create_recording, &$status,
                                       &$language_culture_name, &$duration, &$recording_url) {
    global $CFG, $DB;
    $wiziq_secretacesskey = $CFG->wiziq_secretacesskey;
